...
suite "Stream Transport test suite":
  ...
  for i in 0..<len(addresses):
    ...
    asyncTest prefixes[i] & "close(transport) test":
    ...
    asyncTest prefixes[i] & "readUntil() buffer overflow test":
      ...
      server.start()
      check: await(swarmWorker8(server.local)) == 1
      await server.join()

    asyncTest prefixes[i] & "readLine() buffer overflow test":
      ...
      server.start()
      check: await(swarmWorker7(server.local)) == 1
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "readExactly() unexpected disconnect test":
      ...
      server.start()
      check: await(swarmWorker11(server.local)) == 1
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "readUntil() unexpected disconnect test":
      ...
      server.start()
      check: await(swarmWorker12(server.local)) == 1
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "readLine() unexpected disconnect empty string test":
      ...
      server.start()
      check: await(swarmWorker13(server.local)) == 1
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "Closing socket while operation pending test (issue #8)":
      var subres = 0
      var server = createStreamServer(address, serveClient13, {ReuseAddr})
    ...
    asyncTest prefixes[i] & "readLine() multiple clients with messages (" &
      ...
      server.start()
      check: (await swarmManager1(server.local)) == ClientsCount * MessagesCount
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "readExactly() multiple clients with messages (" &
        $ClientsCount & " clients x " & $MessagesCount & " messages)":
      ...
      server.start()
      check: await(swarmManager2(server.local)) == ClientsCount * MessagesCount
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "readUntil() multiple clients with messages (" &
        $ClientsCount & " clients x " & $MessagesCount & " messages)":
      ...
      server.start()
      check:await(swarmManager3(server.local)) == ClientsCount * MessagesCount
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "write(string)/read(int) multiple clients (" &
        $ClientsCount & " clients x " & $MessagesCount & " messages)":
      ...
      server.start()
      check:await(swarmManager(server.local)) == ClientsCount * MessagesCount
      await server.join()

    asyncTest prefixes[i] & "write(seq[byte])/consume(int)/read(int) multiple clients (" &
         $ClientsCount & " clients x " & $MessagesCount & " messages)":
      var counter = ClientsCount
    ...
    asyncTest prefixes[i] & "writeFile() multiple clients (" & $FilesCount & " files)":
      ...
      when defined(windows):
        if addresses[i].family == AddressFamily.IPv4 and (sizeof(int) == 8):
          check await(testSendFile(addresses[i])) == FilesCount
        else:
          skip()
      else:
        if defined(emscripten) or (sizeof(int) == 4):
          ...
          check await(testSendFile(addresses[i])) == FilesCount
    asyncTest prefixes[i] & "Connection refused test":
      ...
      check await(testConnectionRefused(address))
    asyncTest prefixes[i] & "readOnce() read until atEof() test":
      ...
      server.start()
      check: await(swarmWorker16(server.local)) == 1
      server.stop()
      server.close()
      await server.join()

    asyncTest prefixes[i] & "Connection reset test on send() only":
      when defined(macosx):
        skip()
      else:
    ...
    asyncTest prefixes[i] & "0.0.0.0/::0 (INADDR_ANY) test":
      if addresses[i].family == AddressFamily.IPv4:
        ...
        check await(testAnyAddress())
      else:
        skip()

    asyncTest prefixes[i] & "write() return value test (issue #73)":
      const bigMessageSize = 10 * 1024 * 1024 - 1
    ...
    asyncTest prefixes[i] & "readLine() partial separator test":
    ...
    asyncTest prefixes[i] & "readMessage() test":
      var state = 0
    ...
    asyncTest prefixes[i] & "accept() test":
      ...
      try:
        ...
        if await withTimeout(acceptFut, 5.seconds):
          if acceptFut.finished() and not(acceptFut.failed()):
            ok = (connected == TestsCount) and (connected == accepted)
      finally:
        await server.closeWait()
        if not(isNil(transp)):
          await transp.closeWait()
    ...
    asyncTest prefixes[i] & "accept() and close() race test":
    ...
    asyncTest prefixes[i] & "write() queue notification on close() test":
      ...
      await transp.closeWait()
      await acceptFut
      check res == TestsCount

    asyncTest prefixes[i] & "read() notification on close() test":
      var server = createStreamServer(address, flags = {ReuseAddr, NoPipeFlash})
      var res = false
  ...
  asyncTest "[PIPE] readExactly()/write() test":
  ...
  asyncTest "[IP] bind connect to local address test":
    proc client(server: StreamServer, transp: StreamTransport) {.async: (raises: []).} =
      try:
        await transp.closeWait()
      except CatchableError as exc:
  ...
  asyncTest "[IP] write() cancellation leaks test":
    ...
    proc client(server: StreamServer, transp: StreamTransport) {.async: (raises: []).} =
      try:
        var rbuffer = newSeq[byte](MessageSize)
        for message in messages:
          await transp.readExactly(addr rbuffer[0], len(rbuffer))
          doAssert(rbuffer == message)
        await transp.closeWait()
        server.stop()
        server.close()
      except CatchableError as exc:
  ...
  asyncTest "[IP] accept() cancellation leaks test":
    var
      counter = 0
      exitLoop = false
