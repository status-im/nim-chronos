      ...
      tls =
        try:
          newTLSClientAsyncStream(
            treader, twriter, ha.hostname, flags = session.flags.getTLSFlags()
          )
...
.} =
  ...
  for address in ha.addresses:
    ...
    if not (isNil(transp)):
      let conn = block:
        let res =
          HttpClientConnectionRef.new(session, ha, transp).valueOr:
...
proc getResponse(
  ...
  let bytesRead =
    try:
      ...
      .readUntil(addr buffer[0], len(buffer), HeadersMark)
...
proc post*(
  ...
  HttpClientRequestRef.new(session, url, MethodPost, version, flags, headers, body)
...
proc post*(
  ...
  HttpClientRequestRef.new(session, ha, MethodPost, version, flags, headers, body)
...
proc getBodyReader*(
  ...
  if isNil(response.reader):
    let reader =
      ...
      of HttpClientBodyFlag.Sized:
        let bstream =
          newBoundedStreamReader(response.connection.reader, response.contentLength)
...
proc finish*(response: HttpClientResponseRef) {.async: (raises: [HttpUseClosedError]).} =
