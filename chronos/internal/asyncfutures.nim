...
proc wait*[T](fut: Future[T], timeout = InfiniteDuration): Future[T] =
  ...
  var retFuture = newFuture[T]("chronos.wait()", {FutureFlag.OwnCancelSchedule})
...
proc `or`*[T, Y, E1, E2](
  ...
  type InternalRaisesFutureRaises = union(E1, E2)

  let retFuture = newFuture[void]("chronos.wait()", {FutureFlag.OwnCancelSchedule})
  orImpl(fut1, fut2)
...
proc wait*(fut: InternalRaisesFuture, timeout = InfiniteDuration): auto =
  ...
  let retFuture = newFuture[T]("chronos.wait()", {FutureFlag.OwnCancelSchedule})
