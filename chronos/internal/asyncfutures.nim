...
func isAwaitContinuation(callback: AsyncCallback): bool
...
proc finish(fut: FutureBase, state: FutureState, loc: ptr SrcLoc) =
  ...
  when chronosSyncContinuations:
    if FutureFlag.SyncContinuations in fut.internalFlags:
      proc process(callback: var AsyncCallback) =
        if callback.isAwaitContinuation:
          callSoon(move(callback), immediate = true)

      for i in countdown(fut.internalCallbacks.high, 0):
        process(fut.internalCallbacks[i])
...
func isAwaitContinuation(callback: AsyncCallback): bool =
  callback.function == internalContinue
