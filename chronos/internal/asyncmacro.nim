...
template await*[T: not void](f: Future[T]): var T =
  ## Ensure that the given `Future` is finished, then return a mutable view of
  ## its value. See `readFinished` for details.
  ##
  ...
  when declared(chronosInternalRetFuture):
    ...
    cast[type(f)](chronosInternalRetFuture.internalChild).internalValue
  else:
    unsupported "await is only available within {.async.}"

template await*[T: void](fut: Future[T]) =
  ## Ensure that the given `Future` is finished.
  ##
  ## If the `Future` failed or was cancelled, the corresponding exception will
  ## be raised instead.
  ##
  ...
  when declared(chronosInternalRetFuture):
    ...
    # `child` released by `futureContinue`
    cast[type(fut)](
      chronosInternalRetFuture.internalChild).internalRaiseIfError(fut)
  else:
    unsupported "await is only available within {.async.}"

template await*[T: not void; E](fut: InternalRaisesFuture[T, E]): var T =
  ## Ensure that the given `Future` is finished, then return a mutable view of
  ## its value. See `readFinished` for details.
  ##
  ## If the `Future` failed or was cancelled, the corresponding exception will
  ...
  when declared(chronosInternalRetFuture):
    ...
    cast[type(fut)](chronosInternalRetFuture.internalChild).internalValue
  else:
    unsupported "await is only available within {.async.}"

template await*[T: void; E](fut: InternalRaisesFuture[T, E]) =
  ## Ensure that the given `Future` is finished.
  ##
  ## If the `Future` failed or was cancelled, the corresponding exception will
  ## be raised instead.
