...
template mawait*[T: not void](f: Future[T]): var T =
  ## Ensure that the given `Future` is finished, then return a mutable view of
  ## its value. See `mreadFinished` for details.
  ##
  ...
  when declared(chronosInternalRetFuture):
    ...
    cast[type(f)](chronosInternalRetFuture.internalChild).mvalue()
  else:
    unsupported "await is only available within {.async.}"

template mawait*[T: not void; E](fut: InternalRaisesFuture[T, E]): var T =
  ## Ensure that the given `Future` is finished, then return a mutable view of
  ## its value. See `mreadFinished` for details.
  ##
  ## If the `Future` failed or was cancelled, the corresponding exception will
  ...
  when declared(chronosInternalRetFuture):
    ...
    cast[type(fut)](chronosInternalRetFuture.internalChild).mvalue()
  else:
    unsupported "await is only available within {.async.}"
