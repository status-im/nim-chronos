...
template modifyKQueue(
    changes: var openArray[KEvent],
    index: int,
    nident: uint,
    nfilter: cshort,
    nflags: cushort,
...
proc updateHandle2*[T](s: Selector[T], fd: cint, events: set[Event]): SelectResult[void] =
  let EventsMask = {
...
proc registerTimer*[T](
  ...
  let
    ...
    flags: cushort =
      if oneshot:
        ...
        EV_ADD
    selectorKey =
      SelectorKey[T](ident: fdi32, events: events, param: timeout, data: data)
  ...
  changes.modifyKQueue(
    0, uint(uint32(fdi32)), EVFILT_TIMER, flags, 0, cint(timeout), nil
  )
...
proc registerSignal*[T](s: Selector[T], signal: int, data: T): SelectResult[cint] =
  ...
  let
    ...
    events = {Event.Signal}
    selectorKey =
      SelectorKey[T](ident: fdi32, events: events, param: signal, data: data)
...
proc registerProcess*[T](s: Selector[T], pid: int, data: T): SelectResult[cint] =
  ...
  changes.modifyKQueue(
    0, uint(uint32(pid)), EVFILT_PROC, flags, NOTE_EXIT, 0, fdi32.toPointer()
  )
  if handleEintr(kevent(s.kqFd, addr(changes[0]), cint(1), nil, 0, nil)) == -1:
    s.freeKey(fdi32)
    return err(osLastError())
...
proc registerVnode2*[T](
  ...
  changes.modifyKQueue(
    0, uint(uint32(fd)), EVFILT_VNODE, EV_ADD or EV_CLEAR, fflags, 0, nil
  )
  if handleEintr(kevent(s.kqFd, addr(changes[0]), cint(1), nil, 0, nil)) == -1:
    s.freeKey(fd)
    return err(osLastError())
...
proc prepareKey[T](s: Selector[T], event: KEvent): Opt[ReadyKey] =
  ...
  of EVFILT_READ:
    ...
    if Event.User in pkey.events:
      var data: uint64 = 0
      if handleEintr(osdefs.read(cint(event.ident), addr data, sizeof(uint64))) !=
          sizeof(uint64):
        let errorCode = osLastError()
  ...
  of EVFILT_VNODE:
    ...
    if (event.fflags and NOTE_DELETE) != 0:
      rkey.events.incl(Event.VnodeDelete)
    if (event.fflags and NOTE_WRITE) != 0:
      rkey.events.incl(Event.VnodeWrite)
    if (event.fflags and NOTE_EXTEND) != 0:
      rkey.events.incl(Event.VnodeExtend)
    if (event.fflags and NOTE_ATTRIB) != 0:
      rkey.events.incl(Event.VnodeAttrib)
    if (event.fflags and NOTE_LINK) != 0:
      rkey.events.incl(Event.VnodeLink)
    if (event.fflags and NOTE_RENAME) != 0:
      rkey.events.incl(Event.VnodeRename)
    if (event.fflags and NOTE_REVOKE) != 0:
...
proc selectInto2*[T](
  ...
  for i in 0 ..< eventsCount:
    let rkey =
      s.prepareKey(queueEvents[i]).valueOr:
        continue
    readyKeys[k] = rkey
    inc(k)
...
proc trigger*(ev: SelectEvent) {.raises: [IOSelectorsException].} =
  let res = ev.trigger2()
  if res.isErr():
    raiseIOSelectorsError(res.error())

proc close*(ev: SelectEvent) {.raises: [IOSelectorsException].} =
  let res = ev.close2()
  if res.isErr():
    raiseIOSelectorsError(res.error())
...
): seq[ReadyKey] {.raises: [IOSelectorsException].} =
