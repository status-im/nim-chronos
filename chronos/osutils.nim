...
when defined(windows) or defined(nimdoc):
  ...
  const
    ...
    MaxSignalSuffixLength* =
      MaxSignalEventLength - (len(SignalPrefixName) + Base10.maxLen(uint64) + 2)

type DescriptorFlag* {.pure.} = enum
  CloseOnExec
  NonBlock
...
when defined(windows):
  ...
  proc setDescriptorInheritance*(
      s: WINDESCRIPTOR, value: bool
  ): Result[void, OSErrorCode] =
    var flags = 0'u32
    let fd =
      when s is SocketHandle:
        HANDLE(s)
      else:
        s
  ...
  proc getDescriptorInheritance*(s: WINDESCRIPTOR): Result[bool, OSErrorCode] =
    var flags = 0'u32
    let fd =
      when s is SocketHandle:
        HANDLE(s)
  ...
  proc setDescriptorFlags*(
      s: WINDESCRIPTOR, nonblock, cloexec: bool
  ): Result[void, OSErrorCode] =
  ...
  proc toWideBuffer*(
      s: openArray[char], d: var openArray[WCHAR]
  ): Result[int, OSErrorCode] =
    if len(s) == 0:
      return ok(0)
    let res = multiByteToWideChar(
      CP_UTF8, 0'u32, unsafeAddr s[0], cint(-1), addr d[0], cint(len(d))
    )
    if res == 0:
      err(osLastError())
    else:
  ...
  proc toMultibyteBuffer*(
      s: openArray[WCHAR], d: var openArray[char]
  ): Result[int, OSErrorCode] =
    if len(s) == 0:
      return ok(0)
  ...
  proc toString*(w: LPWSTR): Result[string, OSErrorCode] =
    if isNil(w):
      ...
      let bytesNeeded =
        wideCharToMultiByte(CP_UTF8, 0'u32, w, cint(-1), nil, cint(0), nil, nil)
      if bytesNeeded <= cint(0):
        return err(osLastError())

      var buffer = newString(bytesNeeded)
      let res = wideCharToMultiByte(
        CP_UTF8, 0'u32, w, cint(-1), addr buffer[0], cint(len(buffer)), nil, nil
      )
      if res != bytesNeeded:
        err(osLastError())
      else:
  ...
  proc createOsPipe*(
      readset, writeset: set[DescriptorFlag]
    ...
    while true:
      ...
      let pipeMode =
        osdefs.PIPE_TYPE_BYTE or osdefs.PIPE_READMODE_BYTE or osdefs.PIPE_WAIT
      widePipeName = block:
  ...
  proc setDescriptorFlags*(s: cint, nonblock, cloexec: bool): Result[void, OSErrorCode] =
    ?setDescriptorBlocking(s, not (nonblock))
    ?setDescriptorInheritance(s, not (cloexec))
    ok()
  ...
  proc acceptConn*(
      a1: cint, a2: ptr SockAddr, a3: ptr SockLen, a4: set[DescriptorFlag]
