...
type
  ...
  ReadMessagePredicate* =
    proc(data: openArray[byte]): tuple[consumed: int, done: bool] {.gcsafe, raises: [].}
...
when defined(windows):
  type StreamTransport* = ref object of RootRef
...
type
  ...
  StreamCallback2* =
    proc(server: StreamServer, client: StreamTransport) {.async: (raises: []).}
  ...
  TransportInitCallback* =
    proc(server: StreamServer, fd: AsyncFD): StreamTransport {.gcsafe, raises: [].}
    ## Custom transport initialization procedure, which can allocate inherited
    ## StreamTransport object.

  StreamServer* = ref object of SocketServer ## StreamServer object
    function*: StreamCallback2
      # callback which will be called after new
      # client accepted
    init*: TransportInitCallback
      # callback which will be called before
      # transport for new client

proc getRemoteAddress(
    transp: StreamTransport, address: Sockaddr_storage, length: SockLen
): TransportAddress =
...
proc remoteAddress2*(transp: StreamTransport): Result[TransportAddress, OSErrorCode] =
...
proc localAddress2*(transp: StreamTransport): Result[TransportAddress, OSErrorCode] =
...
proc completePendingWriteQueue(queue: var Deque[StreamVector], v: int) {.inline.} =
  while len(queue) > 0:
    var vector = queue.popFirst()
    if not (vector.writer.finished()):
      vector.writer.complete(v)

proc failPendingWriteQueue(
    queue: var Deque[StreamVector], error: ref TransportError
) {.inline.} =
  while len(queue) > 0:
    var vector = queue.popFirst()
    if not (vector.writer.finished()):
...
when defined(windows):
  ...
  func isConnResetError(err: OSErrorCode): bool {.inline.} =
    case err
    of WSAECONNRESET, WSAECONNABORTED, ERROR_PIPE_NOT_CONNECTED: true
    else: false
        ...
        if WritePending in transp.state:
          ...
          if transp.kind == TransportKind.Socket:
            ...
            if vector.kind == VectorKind.DataBuffer:
              ...
              var ret = loop.transmitFile(
                sock,
                getFileHandle(vector),
                size,
                0'u32,
                cast[POVERLAPPED](addr transp.wovl),
                nil,
            ...
            if vector.kind == VectorKind.DataBuffer:
              ...
              let ret = writeFile(
                pipe,
                cast[pointer](transp.wwsabuf.buf),
                DWORD(transp.wwsabuf.len),
                addr bytesCount,
                cast[POVERLAPPED](addr transp.wovl),
              )
              if ret == 0:
  ...
  proc readStreamLoop(udata: pointer) {.gcsafe, nimcall.} =
    ...
    while true:
      if ReadPending in transp.state:
        ...
        of ERROR_OPERATION_ABORTED, ERROR_CONNECTION_ABORTED, ERROR_BROKEN_PIPE:
        ...
        if transp.state * {ReadEof, ReadClosed, ReadError} == {}:
          ...
          if transp.kind == TransportKind.Socket:
            ...
            let ret = readFile(
              pipe,
              cast[pointer](transp.rwsabuf.buf),
              DWORD(transp.rwsabuf.len),
              addr bytesCount,
              cast[POVERLAPPED](addr transp.rovl),
            )
            if ret == 0:
  ...
  proc newStreamSocketTransport(
      sock: AsyncFD, bufsize: int, child: StreamTransport, flags: set[TransportFlags]
  ): StreamTransport =
    ...
    transp.fd = sock
    transp.flags = flags
    transp.rovl.data = CompletionData(cb: readStreamLoop, udata: cast[pointer](transp))
    transp.wovl.data = CompletionData(cb: writeStreamLoop, udata: cast[pointer](transp))
  ...
  proc newStreamPipeTransport(
      fd: AsyncFD, bufsize: int, child: StreamTransport, flags: set[TransportFlags] = {}
  ): StreamTransport =
    var transp: StreamTransport
    if not (isNil(child)):
      transp = child
  ...
  proc bindToDomain(handle: AsyncFD, family: AddressFamily): Result[void, OSErrorCode] =
    case family
    of AddressFamily.IPv6:
      var saddr: Sockaddr_in6
      saddr.sin6_family = type(saddr.sin6_family)(osdefs.AF_INET6)
      if osdefs.bindSocket(
        SocketHandle(handle), cast[ptr SockAddr](addr(saddr)), sizeof(saddr).SockLen
      ) != 0'i32:
        return err(osLastError())
      ok()
  ...
  .} =
    ...
    if address.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
      ...
      sock = createAsyncSocket2(raddress.getDomain(), SockType.SOCK_STREAM, proto).valueOr:
        ...
        if not (retFuture.finished()):
          if ovl.data.errCode == OSErrorCode(-1):
            if setsockopt(
              SocketHandle(sock),
              cint(SOL_SOCKET),
              cint(SO_UPDATE_CONNECT_CONTEXT),
              nil,
              SockLen(0),
            ) != 0'i32:
              ...
              let transp =
                newStreamSocketTransport(sock, bufferSize, child, transportFlags)
      ...
      pipeContinuation = proc(udata: pointer) {.gcsafe, raises: [].} =
        ...
        if not (retFuture.finished()):
          ...
          if pipeHandle == INVALID_HANDLE_VALUE:
            ...
            let transp = newStreamPipeTransport(AsyncFD(pipeHandle), bufferSize, child)
            # Start tracking transport
            trackCounter(StreamTransportTrackerName)
            retFuture.complete(transp)
        ...
        if server.status notin {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          of OSErrorCode(-1):
            ...
            if not (isNil(server.init)):
              ...
              ntransp =
                newStreamPipeTransport(server.sock, server.bufferSize, transp, flags)
            else:
              ntransp =
                newStreamPipeTransport(server.sock, server.bufferSize, nil, flags)
            # Start tracking transport
            trackCounter(StreamTransportTrackerName)
          ...
          of ERROR_OPERATION_ABORTED:
            ...
            raiseOsDefect(
              osLastError(),
              "acceptPipeLoop(): Unable to accept " & "new pipe connection",
            )
        else:
        ...
        if server.status notin {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          let
            ...
            pipeName = toWideString(pipeAsciiName).valueOr:
              raiseOsDefect(
                error,
                "acceptPipeLoop(): Unable to create name " & "for new pipe connection",
              )
            openMode =
              if FirstPipe notin server.flags:
                server.flags.incl(FirstPipe)
          ...
          if pipeHandle == INVALID_HANDLE_VALUE:
            raiseOsDefect(
              osLastError(), "acceptPipeLoop(): Unable to create " & "new pipe"
            )
          ...
          if wres.isErr():
            raiseOsDefect(
              wres.error(),
              "acceptPipeLoop(): Unable to " & "register new pipe in dispatcher",
            )
          let res = connectNamedPipe(pipeHandle, cast[POVERLAPPED](addr server.aovl))
          if res == 0:
            ...
            if errCode == ERROR_OPERATION_ABORTED:
              ...
              raiseOsDefect(
                errCode, "acceptPipeLoop(): Unable to establish " & "pipe connection"
              )
          break
        else:
          # Server close happens in callback, and we are not started new
    ...
    while true:
      if server.apending:
        ...
        if server.status notin {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          of OSErrorCode(-1):
            if setsockopt(
              ...
              addr server.sock,
              SockLen(sizeof(SocketHandle)),
            ) != 0'i32:
              let errCode = OSErrorCode(wsaGetLastError())
              server.asock.closeSocket()
              raiseOsDefect(
                errCode, "acceptLoop(): Unable to set accept " & "context for socket"
              )
            else:
              var ntransp: StreamTransport
              if not (isNil(server.init)):
                let transp = server.init(server, server.asock)
                ntransp = newStreamSocketTransport(
                  server.asock, server.bufferSize, transp, server.getTransportFlags()
                )
              else:
                ntransp = newStreamSocketTransport(
                  server.asock, server.bufferSize, nil, server.getTransportFlags()
                )
              # Start tracking transport
              trackCounter(StreamTransportTrackerName)
              asyncSpawn server.function(server, ntransp)
          of ERROR_OPERATION_ABORTED:
            ...
            server.asock.closeSocket()
            raiseOsDefect(
              ovl.data.errCode, "acceptLoop(): Unable to accept " & "new connection"
            )
        else:
          # Server close happens in callback, and we are not started new
          # AcceptEx session.
        ...
        if server.status notin {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          server.asock = createAsyncSocket2(
            server.domain, SockType.SOCK_STREAM, Protocol.IPPROTO_TCP
          ).valueOr:
          ...
          let res = loop.acceptEx(
            SocketHandle(server.sock),
            SocketHandle(server.asock),
          ...
          if res == FALSE:
            ...
            if errCode == ERROR_OPERATION_ABORTED:
              ...
              raiseOsDefect(errCode, "acceptLoop(): Unable to accept " & "connection")
          break
        else:
          # Server close happens in callback, and we are not started new
          # AcceptEx session.
  ...
  .} =
    ...
    proc continuationSocket(udata: pointer) {.gcsafe.} =
      ...
      if server.status in {ServerStatus.Stopped, ServerStatus.Closed}:
        ...
        of ERROR_OPERATION_ABORTED:
          ...
          retFuture.fail(getServerUseClosedError())
          server.clean()
        of WSAENETDOWN, WSAENETRESET, WSAECONNABORTED, WSAECONNRESET, WSAETIMEDOUT,
      ...
      if server.status in {ServerStatus.Stopped, ServerStatus.Closed}:
        ...
        of OSErrorCode(-1):
          ...
          if not (isNil(server.init)):
            ...
            ntransp = newStreamPipeTransport(server.sock, server.bufferSize, nil, flags)
    ...
    if server.local.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
      ...
      server.aovl.data =
        CompletionData(cb: continuationSocket, udata: cast[pointer](server))
      server.apending = true
      let res = loop.acceptEx(
        SocketHandle(server.sock),
        SocketHandle(server.asock),
      ...
      if res == FALSE:
        ...
        of ERROR_IO_PENDING:
          discard
        of WSAECONNRESET, WSAECONNABORTED, WSAENETDOWN, WSAENETRESET, WSAETIMEDOUT:
          server.apending = false
      ...
      server.aovl.data =
        CompletionData(cb: continuationPipe, udata: cast[pointer](server))
      server.apending = true
      let res =
        connectNamedPipe(HANDLE(server.sock), cast[POVERLAPPED](addr server.aovl))
      if res == 0:
        let err = osLastError()
        case err
        of ERROR_OPERATION_ABORTED:
  ...
  proc writeStreamLoop(udata: pointer) =
    ...
    while len(transp.queue) > 0:
      ...
      of VectorKind.DataFile:
        ...
        let res =
          sendfile(int(fd), cast[int](vector.buflen), int(vector.offset), nbytes)
  ...
  proc newStreamPipeTransport(
      fd: AsyncFD, bufsize: int, child: StreamTransport
  ): StreamTransport =
    var transp: StreamTransport
    if not (isNil(child)):
      transp = child
  ...
  .} =
    ...
    let sock = createAsyncSocket2(address.getDomain(), SockType.SOCK_STREAM, proto).valueOr:
      case error
      of oserrno.EMFILE:
        retFuture.fail(getTransportTooManyError())
      else:
    ...
    of AddressFamily.IPv4, AddressFamily.IPv6, AddressFamily.Unix:
      ...
      if bindSocket(SocketHandle(sock), cast[ptr SockAddr](addr lsaddr), lslen) != 0:
        sock.closeSocket()
        retFuture.fail(getTransportOsError(osLastError()))
        return retFuture
    of AddressFamily.None:
    ...
    proc continuation(udata: pointer) =
      if not (retFuture.finished()):
        ...
        let transp = newStreamSocketTransport(sock, bufferSize, child, transportFlags)
        # Start tracking transport
        trackCounter(StreamTransportTrackerName)
        retFuture.complete(transp)
    ...
    while true:
      let res = osdefs.connect(SocketHandle(sock), cast[ptr SockAddr](addr saddr), slen)
      if res == 0:
        let transp = newStreamSocketTransport(sock, bufferSize, child, transportFlags)
        # Start tracking transport
        trackCounter(StreamTransportTrackerName)
  ...
  proc acceptLoop(udata: pointer) =
    ...
    let
      ...
      sres =
        acceptConn(cint(server.sock), cast[ptr SockAddr](addr saddr), addr slen, flags)
    if sres.isOk():
      ...
      if rres.isOk():
        let ntransp =
          if not (isNil(server.init)):
            ...
            newStreamSocketTransport(
              sock, server.bufferSize, transp, server.getTransportFlags()
            )
          else:
            newStreamSocketTransport(
              sock, server.bufferSize, nil, server.getTransportFlags()
            )
        trackCounter(StreamTransportTrackerName)
        asyncSpawn server.function(server, ntransp)
      else:
      ...
      if not (retFuture.finished()):
        if server.status in {ServerStatus.Stopped, ServerStatus.Closed}:
          ...
          let
            flags = {DescriptorFlag.CloseOnExec, DescriptorFlag.NonBlock}
            sres = acceptConn(
              cint(server.sock), cast[ptr SockAddr](addr saddr), addr slen, flags
            )
          if sres.isErr():
            let errorCode = sres.error()
            case errorCode
...
proc start2*(server: StreamServer): Result[void, OSErrorCode] =
  ...
  doAssert(
    ...
    "You should not start the server " &
...
.} =
  ...
  if TcpNoDelay in flags:
    mappedFlags.incl(SocketFlags.TcpNoDelay)
...
proc createStreamServer*(
    ...
    udata: pointer = nil,
    dualstack = DualStackType.Auto,
): StreamServer {.raises: [TransportOsError].} =
    ...
    when defined(windows):
      ...
      if host.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
        ...
        let sockres =
          if sock == asyncInvalidSocket:
            ...
            let res = createAsyncSocket2(
              host.getDomain(), SockType.SOCK_STREAM, Protocol.IPPROTO_TCP
            )
            if res.isErr():
              raiseTransportOsError(res.error())
            res.get()
        ...
        if ServerFlags.TcpNoDelay in flags:
          setSockOpt2(sockres, osdefs.IPPROTO_TCP, osdefs.TCP_NODELAY, 1).isOkOr:
            if sock == asyncInvalidSocket:
              discard closeFd(SocketHandle(sockres))
            raiseTransportOsError(error)
        # IPV6_V6ONLY.
        ...
        if bindSocket(SocketHandle(sockres), cast[ptr SockAddr](addr saddr), slen) != 0:
          let err = osLastError()
          if sock == asyncInvalidSocket:
            discard closeFd(SocketHandle(sockres))
        ...
        if getsockname(SocketHandle(sockres), cast[ptr SockAddr](addr saddr), addr slen) !=
            0:
          let err = osLastError()
          if sock == asyncInvalidSocket:
            discard closeFd(SocketHandle(sockres))
        ...
        if sock == asyncInvalidSocket:
          let proto =
            if host.family == AddressFamily.Unix:
              ...
              Protocol.IPPROTO_TCP
          # TODO (cheatfate): `valueOr` generates weird compile error.
          let res = createAsyncSocket2(host.getDomain(), SockType.SOCK_STREAM, proto)
          if res.isErr():
            raiseTransportOsError(res.error())
          res.get()
        else:
      ...
      if osdefs.bindSocket(SocketHandle(sockres), cast[ptr SockAddr](addr saddr), slen) !=
          0:
        let err = osLastError()
        if sock == asyncInvalidSocket:
          discard unregisterAndCloseFd(sockres)
  ...
  sres.loopFuture = asyncloop.init(
    Future[void].Raising([]), "stream.transport.server", {FutureFlag.OwnCancelSchedule}
  )
  sres.udata = udata
...
.} =
  proc wrap(server: StreamServer, client: StreamTransport) {.async: (raises: []).} =
    try:
  ...
  createStreamServer(
    host, wrap, flags, sock, backlog, bufferSize, child, init, udata, dualstack
  )
...
template fastWrite(transp: auto, pbytes: var ptr byte, rbytes: var int, nbytes: int) =
    ...
    if transp.queue.len == 0:
      while rbytes > 0:
        let res =
          case transp.kind
          of TransportKind.Socket:
            osdefs.send(SocketHandle(transp.fd), pbytes, rbytes, MSG_NOSIGNAL)
          of TransportKind.Pipe:
...
proc write*(
    transp: StreamTransport, pbytes: pointer, nbytes: int
): Future[int] {.async: (raw: true, raises: [TransportError, CancelledError]).} =
  ...
  var vector = StreamVector(
    kind: DataBuffer, writer: retFuture, buf: pbytes, buflen: rbytes, size: nbytes
  )
...
proc write*(
    transp: StreamTransport, msg: string, msglen = -1
): Future[int] {.async: (raw: true, raises: [TransportError, CancelledError]).} =
  ...
  let written = nbytes - rbytes # In case fastWrite wrote some
...
proc write*[T](
    transp: StreamTransport, msg: seq[T], msglen = -1
): Future[int] {.async: (raw: true, raises: [TransportError, CancelledError]).} =
...
proc writeFile*(
    transp: StreamTransport, handle: int, offset: uint = 0, size: int = 0
): Future[int] {.async: (raw: true, raises: [TransportError, CancelledError]).} =
  ...
  when defined(windows):
    if transp.kind != TransportKind.Socket:
      retFuture.fail(newException(TransportNoSupport, "writeFile() is not supported!"))
      return retFuture
  transp.checkClosed(retFuture)
  transp.checkWriteEof(retFuture)
  var vector = StreamVector(
...
template readLoop(name, body: untyped): untyped =
  ...
  while true:
    if ReadClosed in transp.state:
      raise
        newException(TransportUseClosedError, "Attempt to read data from closed stream")
...
proc readOnce*(
    transp: StreamTransport, pbytes: pointer, nbytes: int
): Future[int] {.async: (raises: [TransportError, CancelledError]).} =
...
proc readUntil*(
    transp: StreamTransport, pbytes: pointer, nbytes: int, sep: seq[byte]
): Future[int] {.async: (raises: [TransportError, CancelledError]).} =
...
proc readLine*(
    transp: StreamTransport, limit = 0, sep = "\r\n"
): Future[string] {.async: (raises: [TransportError, CancelledError]).} =
...
proc read*(
    transp: StreamTransport
): Future[seq[byte]] {.async: (raises: [TransportError, CancelledError]).} =
...
proc readMessage*(
    transp: StreamTransport, predicate: ReadMessagePredicate
) {.async: (raises: [TransportError, CancelledError]).} =
...
proc running*(transp: StreamTransport): bool {.inline.} =
  ...
  (
    {ReadClosed, ReadEof, ReadError, WriteClosed, WriteEof, WriteError} * transp.state ==
    {}
  )
...
proc shutdownWait*(
  ...
  when defined(windows):
    ...
    let res =
      loop.disconnectEx(SocketHandle(transp.fd), cast[POVERLAPPED](povl), 0'u32, 0'u32)
    if res == FALSE:
      let err = osLastError()
      case err
      of ERROR_IO_PENDING:
...
proc fromPipe*(
    fd: AsyncFD, child: StreamTransport = nil, bufferSize = DefaultStreamBufferSize
): StreamTransport {.raises: [TransportOsError].} =
  ## Create new transport object using pipe's file descriptor.
  ##
  ## ``bufferSize`` is size of internal buffer for transport.
