...
when defined(windows):
  ...
  template setReaderWSABuffer(t: untyped) =
    (t).rwsabuf.buf = cast[cstring](cast[uint](addr t.buffer[0]) + uint((t).roffset))
  ...
  proc newStreamSocketTransport(
      sock: AsyncFD, bufsize: int, child: StreamTransport
  ): StreamTransport =
  ...
  .} =
    ...
    if address.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
      ...
      sock =
        createAsyncSocket2(raddress.getDomain(), SockType.SOCK_STREAM, proto).valueOr:
      ...
      if localAddress != TransportAddress():
        ...
        if bindSocket(
          SocketHandle(sock), cast[ptr SockAddr](addr localAddr), localAddrLen
        ) != 0:
          sock.closeSocket()
      ...
      pipeContinuation = proc(udata: pointer) {.gcsafe, raises: [].} =
        ...
        if not (retFuture.finished()):
          let
            ...
            pipeName =
              toWideString(pipeAsciiName).valueOr:
                retFuture.fail(getTransportOsError(error))
                return
          ...
          of OSErrorCode(-1):
            if setsockopt(
              ...
              if not (isNil(server.init)):
                ...
                ntransp = newStreamSocketTransport(server.asock, server.bufferSize, nil)
              # Start tracking transport
              trackCounter(StreamTransportTrackerName)
              asyncSpawn server.function(server, ntransp)
          of ERROR_OPERATION_ABORTED:
  ...
  .} =
    ...
    proc continuationPipe(udata: pointer) {.gcsafe.} =
      ...
      if server.status in {ServerStatus.Stopped, ServerStatus.Closed}:
        ...
        of OSErrorCode(-1):
          ...
          server.sock =
            server.createAcceptPipe().valueOr:
    ...
    if server.local.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
      ...
      var loop = getThreadDispatcher()
      server.asock =
        createAsyncSocket2(server.domain, SockType.SOCK_STREAM, Protocol.IPPROTO_TCP)
        .valueOr:
          case error
  ...
  .} =
    ...
    let sock =
      createAsyncSocket2(address.getDomain(), SockType.SOCK_STREAM, proto).valueOr:
        case error
        of oserrno.EMFILE:
          retFuture.fail(getTransportTooManyError())
        else:
  ...
  when defined(nimdoc):
    ...
    if host.family in {AddressFamily.IPv4, AddressFamily.IPv6}:
      ...
      if ServerFlags.TcpNoDelay in flags:
        setSockOpt2(serverSocket, osdefs.IPPROTO_TCP, osdefs.TCP_NODELAY, 1).isOkOr:
          if sock == asyncInvalidSocket:
            discard closeFd(SocketHandle(serverSocket))
          raiseTransportOsError(error)
      # IPV6_V6ONLY.
      ...
      if bindSocket(SocketHandle(serverSocket), cast[ptr SockAddr](addr saddr), slen) !=
          0:
        let err = osLastError()
        if sock == asyncInvalidSocket:
          discard closeFd(SocketHandle(serverSocket))
      ...
      if getsockname(
        SocketHandle(serverSocket), cast[ptr SockAddr](addr saddr), addr slen
      ) != 0:
        let err = osLastError()
        if sock == asyncInvalidSocket:
          discard closeFd(SocketHandle(serverSocket))
    ...
    host.toSAddr(saddr, slen)
    if osdefs.bindSocket(
      SocketHandle(serverSocket), cast[ptr SockAddr](addr saddr), slen
    ) != 0:
      let err = osLastError()
      if sock == asyncInvalidSocket:
        discard unregisterAndCloseFd(serverSocket)
      ...
      while rbytes > 0:
        ...
        if res > 0:
          ...
          if rbytes == 0:
            ...
            return retFuture # Not all bytes written - keep going
        else:
          let err = osLastError()
          case err
          of oserrno.EWOULDBLOCK:
...
proc write*(
    transp: StreamTransport, msg: sink string, msglen = -1
): Future[int] {.async: (raw: true, raises: [TransportError, CancelledError]).} =
...
proc write*[T](
    transp: StreamTransport, msg: sink seq[T], msglen = -1
): Future[int] {.async: (raw: true, raises: [TransportError, CancelledError]).} =
...
proc atEof*(transp: StreamTransport): bool {.inline.} =
  ...
  (transp.offset == 0) and (ReadEof in transp.state) and (ReadPaused in transp.state)
