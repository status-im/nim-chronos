...
Middleware could also modify HTTP server response, and these changes will be visible to all handlers (either middlewares or the original request handler). This can be done using the following helpers:

```nim
  proc updateResponse*(response: HttpResponseRef, status: HttpCode,
                       version: HttpVersion, headers: HttpTable): HttpResultMessage[void]

  proc updateResponse*(response: HttpResponseRef, status: HttpCode,
                       headers: HttpTable): HttpResultMessage[void]

  proc updateResponse*(response: HttpResponseRef,
                       headers: HttpTable): HttpResultMessage[void]
```

As you can see all the HTTP response parameters could be modified: response status, version and response headers.

Middleware could also block HTTP request, and this can be done using the following helpers:

```nim
  proc codeResponse*(code: HttpCode): HttpResponseRef

  proc dropResponse*(): HttpResponseRef
```

Middleware could also pass control to the next middleware or the original request handler.
